...
class PlateRecognition(object):
    ...
    def _detect(self, vehicles, stream_no: int, frame):
        ...
        #print("Vehicles detected count", len(vehicles))
        ...
        for car in vehicles:
            ...
            if with_offset:
                ...
                #print("width of plate", (plate_maxx-plate_minx))
                if ((plate_maxx-plate_minx) < 70):
                    continue
            #elif((plate_maxx-plate_minx) < 50):
            elif((plate_maxx-plate_minx) < 40):
                #print("width of plate", (plate_maxx-plate_minx))
                #print("width of plate", (plate_maxx-plate_minx))
                continue
            ...
            if ((lpr_results is not None)):    
                ...
                #plate_prefix = f'{سبت 07 10:17:16 nvidia-desktop detect_ivms.sh[5302]: TypeError: lpr_results.decoded_label["prefix_label"]}'.strip()
                plate_type = self.PLATE_TYPE_PRIVATE
                plate_state = f'{lpr_results["StateLong"]}'.strip()
                plate_country = f'{lpr_results["CountryLong"]}'.strip()
                plate_no = f'{plate_state} {plate_num}'.strip()
